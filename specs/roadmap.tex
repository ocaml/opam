...
is available in the {\tt default} repository, then
\verb+$opam/repo/index+ will contain:
...
\item \verb+$-rm $REPO$+ deletes \verb+$opam/repo/$REPO+ and removes
  ...
  Then, for each package in \verb+$opam/repo/index+ it update the link
  between packages and repositories (ie. it either deletes packages or
  associates them to an other repository)
