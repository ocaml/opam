...
\begin{verbatim}
    $ opam init [-kind $KIND] [-ocamlc $OCAMLC] $REPO $ADDRESS
\end{verbatim}
...
\item \verb+$KIND+ is the kind of OPAM repository (default is {\tt
  http});
\item \verb+$OCAMLC+ is the complete filename leading to the
  {\tt ocamlc} compiler (default is the first binary found from
  \verb+$path+, if found);
