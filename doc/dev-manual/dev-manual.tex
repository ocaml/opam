...
\item The {\tt license}, {\tt homepage}, {\tt doc}, {\tt bug-reports} and {\tt
    authors} fields are optional. {\tt doc} should be the address of the online
...
\item The optional {\tt features} field is used to advertise optional package
  features that may depend on the environment. Each feature has an identifier, a
  user-readable description and a filter. The filter is evaluated and the
  resulting boolean value is bound to the identifier, that can typically be used
  in the {\tt build} field to trigger \verb+./configure+ options. For example,
  package {\tt foo} may have an {\tt overdrive} feature that requires that both
  {\tt bar} and {\tt baz.2} are installed. You would write it like this:

  \begin{Verbatim}
build: [ ["./configure" "-prefix=%{prefix}%" "-%{overdrive:enable}%-overdrive"]
         ... ]
depopts: [ "bar" "baz" ]
features: [ overdrive "Cool feature" { bar:installed & baz:version = "2" } ]
\end{Verbatim}
...
%XXX deprecate ? Use with features ?
...
\item User-defined variables, as defined in {\tt *.config} files (see
  ...
  variables.
\item User-defined variables using the {\tt features} field of OPAM files. See
  \ref{file:opam}. These behave like boolean package-local variables.
\end{enumerate}
