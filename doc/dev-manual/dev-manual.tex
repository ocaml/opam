...
\item The content of {\tt build} is the list of commands to run in
  ...
  \S\ref{file:config} and \S\ref{file:install}).
   ...
   or not. A typical example is OS-related filters, where we can choose
...
\item {\tt build-test} is optional and describes how the documentation
  is built.
...
\item OPAM will try to install all the file in sequence, and it will
  ...
  should start by \verb+?+. {\rm Remark:} it is much cleaner if the
  underlying build-system can generate the right {\tt .install} files,
  containing the existing files only.
\end{itemize}
