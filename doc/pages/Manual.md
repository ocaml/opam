...
- [`/packages/<pkgname>/<pkgname>.<version>/files/`](#files): contains files
  ...
  it gets used.
- `/archives/<pkgname>.<version>+opam.tar.gz`: cached package archives
  (as generated by running `opam-admin make`)
...
Also see the [`extra-sources:`](#opamfield-extra-sources) opam field, which has
a similar behaviour and is processed before the `files/` are copied.

Note that, at the moment, repository archives generated by `opam-admin make` in
`archives/` already include these overlay files: the archive is created from the
package source after the above operations have been done.
