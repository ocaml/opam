...
let pull_from_upstream
    label ?from_source ?full_fetch ?(working_dir=false) ?subpath
  ...
  (if working_dir then B.sync_dirty ?subpath destdir url
   ...
   in
   pull ?from_source ?full_fetch ?cache_dir ?subpath destdir cksum url
  )
  @@| function
...
let pull_from_mirrors label ?from_source ?full_fetch ?working_dir ?subpath
    cache_dir destdir checksums urls =
  let rec aux = function
    | [] -> invalid_arg "?from_source pull_from_mirrors: empty mirror list"
    | [url] ->
      pull_from_upstream label ?from_source ?full_fetch ?working_dir ?subpath
        cache_dir destdir checksums url
      @@| fun r -> url, r
    | url::mirrors ->
      pull_from_upstream label ?from_source ?full_fetch ?working_dir ?subpath
...
(* handle subpathes *)
let pull_tree_t
    ?from_source ?full_fetch ?cache_dir ?(cache_urls=[]) ?working_dir
      ...
      let pull =
        ...
        fun label checksums remote_urls ->
          pull_from_mirrors (OpamStd.Option.default label label0)
            ?from_source ?full_fetch ?working_dir ?subpath cache_dir destdir
...
let pull_tree label
    ?from_source ?full_fetch ?cache_dir ?(cache_urls=[]) ?working_dir ?subpath
    local_dirname  =
  pull_tree_t ?from_source ?full_fetch ?cache_dir ~cache_urls ?working_dir
    [label, local_dirname, subpath]
