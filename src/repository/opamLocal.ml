...
let rsync ?(args=[]) ?(exclude_vcdirs=true) src dst =
  ...
  if not(remote || Sys.file_exists src) then
    Done (Not_available (
        Generic_failure { short_reason = None; long_reason = src }))
...
let rsync_dirs ?args ?exclude_vcdirs url dst =
  ...
  then
    Done (Not_available
            (Generic_failure {
                short_reason = None;
                long_reason = Printf.sprintf
                    "Directory %s does not exist" src_s; }))
  else
  ...
  | Result _ ->
    if OpamFilename.exists_dir dst then Result dst
                          ...
                          { short_reason = None; long_reason = dst_s })
...
let rsync_file ?(args=[]) url dst =
  ...
  if not (is_remote url || OpamFilename.(exists (of_string src_s))) then
    Done (Not_available (Generic_failure
                           { short_reason = None; long_reason =  src_s }))
    ...
    (OpamFilename.mkdir (OpamFilename.dirname dst);
     ...
     @@| function
     | None -> Not_available (Generic_failure
                                { short_reason = None; long_reason = src_s })
     | Some [] -> Up_to_date dst
     | Some [_] ->
