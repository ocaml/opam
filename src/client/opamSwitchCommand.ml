...
let list ~print_short ~installed ~all =
  ...
  let descr c =
    if c = OpamCompiler.system then
      ...
      OpamFile.Descr.safe_read (OpamPath.compiler_descr c) in
  ...
  let officials, patches =
    OpamCompiler.Set.fold (fun comp (officials, patches) ->
        try
          let c = OpamFile.Comp.read (OpamPath.compiler_comp comp) in
...
let remove_t ?(confirm = true) t =
  ...
  let comp_dir = OpamPath.Switch.root t.switch in
  ...
  if Some t.switch =
     OpamStd.Option.Op.(OpamStateConfig.load (OpamPath.config ()) >>|
...
let install_compiler ~quiet switch compiler =
  ...
  with e ->
    ...
    OpamStateConfig.write (OpamPath.config ()) t.switch_global.config;
    raise e
    ...
    try
      let _ =
        OpamPackage.Name.Map.iter (fun name pin ->
            let overlay_dir =
              OpamPath.Switch.Overlay.package t.switch name
            in
...
let export filename =
  ...
  let st = S.safe_read (OpamPath.Switch.state switch) in
  match filename with
...
let reinstall_gt gt switch =
  ...
  let keep_dirs = [
    OpamPath.Switch.Overlay.dir switch;
    OpamPath.Switch.backup_dir switch;
  ] in
  let keep_files = [
    OpamPath.Switch.lock switch;
  ] in
  List.iter
...
let with_backup gt switch f =
  let state_file = OpamPath.Switch.state switch in
  let backup_state_file = OpamPath.backup () in
