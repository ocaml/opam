...
let help_sections cli =
  ...
  @ [
    ...
    `P "Kate Deplaix       <kit-ty-kate@exn.st>"; `Noblank;
...
let complete_packages ?(which=`Installed ) ?(extras=[]) () =
  complete_with_switch @@ fun st ~token ->
    ...
    let directives = List.filter_map
      ...
      (OpamPackage.Name.Set.to_seq packages |> List.of_seq)
    in
...
let filename =
  ...
  Arg.Conv.make ~completion:Arg.Completion.complete_files ~docv:"A file" ~parser ~pp ()
