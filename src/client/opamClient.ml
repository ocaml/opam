...
module API = struct
  ...
  let preprocessed_request t full_orphans orphan_versions
    ...
    let base_packages =
      let comp = OpamState.compiler_comp t ct.compiler in
  ...
  let install_t ?ask atoms add_to_roots ~deps_only ~upgrade t =
    ...
    if ct.installed_roots <> current_roots then (
      ...
      let file = OpamPath.Switch.installed_roots t.root ct.switch in
      OpamFile.Installed_roots.write file ct.installed_roots;
    );
