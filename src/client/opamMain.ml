...
let upgrade =
  ...
  let upgrade global_options build_options fixup atoms =
    ...
    if fixup then
      ...
      else `Ok (OpamClient.fixup gt)
    else
      `Ok (OpamClient.upgrade gt atoms) in
...
let source =
  ...
  let source global_options atom dev_repo pin dir =
    ...
    if pin then
      ...
      in
      OpamClient.PIN.pin gt nv.name ~version:nv.version
        (Some pin_option)
  in
  Term.(pure source
    ...
    if OpamSystem.command_exists ~env command then
      ...
      | Some sw ->
        ...
        then
          ...
          OpamClient.install gt [OpamSolution.eq_atom_of_package nv]
