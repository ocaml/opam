...
(** Scans for package definition files in a directory, and selects a compiler
    ...
    Returns a couple: the corresponding atoms and a boolean that is true if the
    compiler is a local package. If no compiler matches, prints a warning, and
    returns [None] after user confirmation. *)
