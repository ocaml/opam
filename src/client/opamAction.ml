...
let make_command st opam ?dir ?text_command (cmd, args) =
  ...
  OpamSystem.make_command ~env ~name ?dir ~text
    ...
    ~verbose:(OpamConsole.verbose () || OpamPackage.Name.(Set.mem (of_string name) OpamStateConfig.(!r.verbose_on)))
    cmd args
...
let build_package t ?(test=false) ?(doc=false) ?(dev_setup=false) build_dir nv =
  ...
  | None, None ->
    if commands <> [] && (OpamConsole.verbose () || OpamPackage.Name.(Set.mem (of_string name) OpamStateConfig.(!r.verbose_on))) then
