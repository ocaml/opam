...
let edit t name =
  ...
  let installed_nv =
    ...
    with Not_found -> None
  in
  let file = OpamPath.Switch.Overlay.opam t.switch name in
  let temp_file = OpamPath.Switch.Overlay.tmp_opam t.switch name in
  let orig_opam =
  ...
  let cur_version, no_changes =
    try
      ...
      if OpamConsole.confirm "Proceed ?" then
        (OpamFilename.remove
           (OpamPath.Switch.Overlay.tmp_opam t.switch name);
         version, no_changes)
