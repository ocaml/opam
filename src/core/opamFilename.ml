...
let parse_patch_file ~dir patch_file =
  OpamSystem.parse_patch_file ~dir:(Dir.to_string dir) (to_string patch_file)
...
let patch ~allow_unclean patch_source dir =
  ...
  try
    ...
    | `Patch_file p ->
      let diffs = parse_patch_file ~dir:(Dir.to_string dir) p in
