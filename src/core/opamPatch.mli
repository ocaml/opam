...
module type FS_ABSTR = sig
  ...
  (* For directories, we need to add an end slash *)
  ...
  (* Function called when a patch application fails
...
(** [patch FS_ABSTRACTION ~allow_unclean ?patch_filename ~dir diffs] applies a
    patch to given element by [FS_ABSTRACTION].

    @param FS_ABSTRACTION is a module that defines the element that we are
