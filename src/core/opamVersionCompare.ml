...
let extract_revision x =
  try
    ...
    let before = (0, di) in
    let after = (di+1, String.length x) in
    (before,after)
  with
    | Not_found -> ((0, String.length x), (0, 0))
...
let compare_chunks x (xi, xl) y (yi, yl) =
  let rec loop_lexical (xi : int) (yi : int) =
    assert (xi <= xl && yi <= yl);
  ...
  and compare_numerical (xi : int) (yi : int) =
    ...
    in if comp = 0
      then (* both numerical parts have same length: compare digit by digit *)
        ...
        comp
  and loop_numerical (xi : int) (yi : int) (yn : int) =
...
let compare (x : string) (y : string) =
  ...
  in
    ...
    let u_comp = compare_chunks x u1 y u2 in
    if u_comp <> 0 then normalize_comp_result u_comp
    else normalize_comp_result (compare_chunks x r1 y r2)
