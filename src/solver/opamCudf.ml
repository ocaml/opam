...
let extract_explanations packages cudfnv2opam reasons : explanation list =
  ...
  let rec is_included_in_formula formula1 formula2 =
    ...
    | Atom (name1, vformula1), Atom (name2, vformula2) ->
      if OpamPackage.Name.equal name1 name2 then
        match OpamFormula.simplify_version_formula (Or (vformula1, vformula2)) with
        | Some vformula -> Some (Atom (name1, vformula))
        | None -> None
      else
        None
    | Block x, y | x, Block y -> is_included_in_formula x y
    | And (x1, x2), And (y1, y2) ->
        is_included_in_formula x1 y1 >>= fun z1 ->
        is_included_in_formula x2 y2 >>= fun z2 ->
        Some (And (z1, z2))
    | Or (x1, x2), Or (y1, y2) ->
        is_included_in_formula x1 y1 >>= fun z1 ->
        is_included_in_formula x2 y2 >>= fun z2 ->
    ...
    let simplify_cst f f' =
      ...
      | `Formula (bold, f), `Formula (bold', f') ->
        if Bool.equal bold bold' then
          match is_included_in_formula f f' with
          ...
          List.fold_left (fun ((s, l, b) as x, xs) -> function
              | `Conflict ((s', l', b') as y) ->
                if OpamStd.Option.equal String.equal s s' &&
                  ...
                  | new_l -> ((s, new_l, b), xs)
                  | exception Invalid_argument _ -> (x, `Conflict y :: xs)
                else
                  (x, `Conflict y :: xs)
          ...
          List.fold_left (fun ((csp, fdeps) as x, xs) -> function
              ...
              | `Missing ((csp', fdeps') as y) ->
                (match List.map2 simplify_cst csp csp' with
                 | simplified_csp ->
                   (match is_included_in_formula fdeps fdeps' with
                    | Some simplified_formula ->
                      ((simplified_csp, simplified_formula), xs)
                    | None -> (x, `Missing y :: xs))
                 | exception Invalid_argument _ -> (x, `Missing y :: xs)))
            (x, []) xs
        in
  ...
  let explanations =
    let rec reduce_invariant_msg acc had_invariant = function
      | [] -> acc
      | `Conflict (s, l, b)::xs ->
        let l = List.map cst_to_string l in
