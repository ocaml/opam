...
let resolve_depext_availability ?env global_config repos_sys_available_pkgs
  ...
  | None -> OpamSysInteract.packages_status ?env global_config syspkg_set
  | Some (OpamSysPkg.Available sys_pkgs) ->
  ...
  | Some OpamSysPkg.Suppose_available ->
...
let update_sys_packages pkgs st =
  ...
  if OpamSysPkg.Set.is_empty depexts_s then
    ...
    | Some (OpamSysPkg.Available available_pkgs) ->
      ...
      then
        ...
        st
    | Some Suppose_available | None -> st
