...
val download_package: state -> switch -> package ->
...
val extract_package: state -> switch -> generic_file option -> package -> unit
...
val build_package:
  state -> switch -> generic_file option -> package -> exn option OpamProcess.job
...
val install_package:
  state -> switch -> package -> exn option OpamProcess.job

(** Find out if the package source is needed for uninstall *)
val removal_needs_download: state -> switch -> package -> bool

(** Remove a package. *)
val remove_package:
  state -> switch -> ?keep_build:bool -> ?silent:bool ->
...
val cleanup_package_artefacts: state -> switch -> package -> unit

(** Compute the set of packages which will need to be downloaded to apply a
    solution. Takes a graph of atomic actions. *)
val sources_needed: state -> switch -> OpamSolver.ActionGraph.t -> package_set
