...
let abort_if_set var shell =
  (* Each of these incantations aborts the currently running script if [var] is
     unset or empty ("null") *)
  match shell with
  | SH_zsh | SH_bash | SH_sh -> Printf.sprintf "test -z \"${%s:+x}\" || return\n" var
  | SH_csh -> Printf.sprintf "if ( ${?%s} ) then\n  if ( \"$%s\" != \"\") exit\nendif\n" var var
  | SH_pwsh _ -> Printf.sprintf "if ($env:%s -ne $null -and $env:%s -ne '') { return }\n" var var
  | SH_fish -> Printf.sprintf "test -z \"$%s\"; or return\n" var
  | SH_cmd -> Printf.sprintf "if defined %s if \"%%%s%%\" neq \"\" goto :EOF\n" var var
