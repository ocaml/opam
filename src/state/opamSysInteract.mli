...
(* [available_packages ?env config pkgs] queries the system to determine which of the
    ...
    or known to the system's package manager). This includes both real and virtual packages.

    The result is an {!OpamSysPkg.available} type, which may either be a concrete list
