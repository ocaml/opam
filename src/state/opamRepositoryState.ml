...
module Cache = struct
  type t = {
    ...
    cached_sys_available_pkgs: OpamSysPkg.availability_mode option;
  }
...
let load lock_kind gt =
  ...
  | None ->
    ...
    let repos_sys_available_pkgs =
      if not (OpamFile.Config.depext gt.config) then
        None
      else
        try
          Some (OpamSysInteract.available_packages ~env:gt.global_variables
